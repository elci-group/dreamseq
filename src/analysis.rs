...
impl Analysis {
    pub(crate) fn merge(&mut self, other: Self) {
        merge_by(
            &mut self.model_failures,
            other.model_failures,
            |item| (item.model.clone(), item.issue.clone()),
            |existing, finding| {
                existing.frequency = existing.frequency.saturating_add(finding.frequency);
            },
        );
        merge_by(
            &mut self.harness_friction,
            other.harness_friction,
            |item| (item.harness.clone(), item.issue.clone()),
            |existing, finding| {
                existing.severity = existing.severity.max(finding.severity);
            },
        );
        merge_by(
            &mut self.missing_tooling,
            other.missing_tooling,
            |item| (item.tool_name.clone(), item.purpose.clone()),
            |existing, finding| {
                existing.estimated_value = existing.estimated_value.max(finding.estimated_value);
            },
        );
        merge_by(
            &mut self.workflow_bottlenecks,
            other.workflow_bottlenecks,
        ...
        merge_by(
            &mut self.repeated_commands,
            other.repeated_commands,
        ...
        merge_by(
            &mut self.repeated_prompts,
            other.repeated_prompts,
            |item| item.prompt_pattern.clone(),
            |existing, finding| {
                existing.frequency = existing.frequency.saturating_add(finding.frequency);
            },
        );
        ...
        merge_by(
            &mut self.automation_opportunities,
            other.automation_opportunities,
            |item| item.description.clone(),
            |existing, finding| {
